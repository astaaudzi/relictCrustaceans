...
#three lakes checked with handnet only
...
#first with the main sample size of lakes that have been assessed both in 20th and 21st centuries - dataset 'amp'
...
#results with max and average depths are very similar, but average depths gives lower AIC values so we go with them. The average depth model suggested we drop lake are first
...
## Sensitivity analyses: full data set

```{r}
## Now for sensitivity analyses we repeat same analyses with the full data set 
AM1_f <- glmmTMB(Rel_amp ~ Period + logFlow.std + logArea.std + aDepth.std + PC +
                          (1|fLake),
                          compois(link = "log"),
                          data = ampFull)
drop1(AM1_f)
...
# <none>         218.60
# Period       1 218.18
# logFlow.std  1 224.50
# logArea.std  1 217.24
# aDepth.std   1 246.06
# PC           1 217.05 <- drop

#we can do the same with the maximum depth 
AM1max_f <- glmmTMB(Rel_amp ~ Period + logFlow.std + logArea.std + mDepth.std + PC +
                          (1|fLake),
                          compois(link = "log"),
                          data = ampFull)
drop1(AM1max_f)
...
# <none>         224.54
# Period       1 224.29
# logFlow.std  1 229.10
# logArea.std  1 222.56
# mDepth.std   1 246.06
# PC           1 222.56 <- drop this or log Area (identical values). Again average depth gives lower AIC so we go with that

#continue with the average depth model after dropping Ponto-Caspian crustaceans 

AM1a_f <- glmmTMB(Rel_amp ~ Period + logFlow.std + logArea.std + aDepth.std +
                 (1|fLake),
               compois(link = "log"),
               data = ampFull)
...
# <none>         217.05
# Period       1 218.56
# logFlow.std  1 222.52
# logArea.std  1 215.49 <- drop
# aDepth.std   1 244.06

AM1b_f <- glmmTMB(Rel_amp ~ Period + logFlow.std + aDepth.std +
                  (1|fLake),
                compois(link = "log"),
                data = ampFull)
...
#             Df    AIC
# <none>         215.49 <- cannot improve fit by dropping further variables
# Period       1 216.70
# logFlow.std  1 220.59
# aDepth.std   1 242.15

# Improve fit with different link function?
AM1c_f <- glmmTMB(Rel_amp ~ Period + logFlow.std + aDepth.std +
                          (1|fLake),
                          compois(link = "sqrt"),
                          data = ampFull)
...
#      df      AIC
# AM1b  6 215.488
# AM1c  6 211.997 <- better fit

# The optimal model is
AM2_f <- glmmTMB(Rel_amp ~ Period + logFlow.std + aDepth.std + 
                          (1|fLake),
                          compois(link = "sqrt"),
                          data = ampFull)
```
...
# The optimal model is
AM2_t <- glmmTMB(Rel_amp ~ Period + logFlow.std + aDepth.std + 
                          (1|fLake),
                          compois(link = "sqrt"),
                          data = ampTest)
...
plot1 <- plot_model(AM2,
                    ...
                    axis.title = c("Log flowthrough (standardised)",
                                   "Relative abundance of relic amphipods"),
...
plot2 <- plot_model(AM2,
                    ...
                    show.values = F) + My_theme +
  scale_y_continuous(limits = c(0, 5), breaks = c(0, 1, 2, 3, 4)) +
  scale_x_continuous(limits = c(-1.9, 2.6), breaks = c(-1.9, -0.4, 1.1, 2.6)) 
