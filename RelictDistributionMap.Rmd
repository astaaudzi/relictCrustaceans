...
p <- ggplot() +
  ...
  geom_sf_text(data = lt_cities, aes(label = NAME),
               color = "black", size = 2, nudge_x = 0.1, nudge_y = 0.1) +
...
ggsave(europe_map, filename = "astaBugPlot/europe.png", units = "px", width = 750, height = 750)
...
myPlot <- image_graph(width = 2000, height = 2000, res = 200)
print(relicta)
dev.off()

# read in the image to overlay
mysis <- image_read("astaBugPlot/Mysis.tif")
mysis <- image_scale(mysis, "20%")

mono <- image_read("astaBugPlot/Monoporeia.tif")
mono <- image_scale(mono, "20%")

palla <- image_read("astaBugPlot/Pallaseopsis.tif")
palla <- image_scale(palla, "20%")

europe <- image_read("astaBugPLot/europe.png")

# overlay the image on the plot
myPlot <- image_composite(myPlot, mysis, offset = "+100+750")
myPlot <- image_composite(myPlot, mono, offset = "+1100+700")
myPlot <- image_composite(myPlot, palla, offset = "+100+1550")
myPlot <- image_composite(myPlot, europe, offset = "+1100+950")


image_write(myPlot, path = "astaBugPlot/Fig1.png", format = "png")
